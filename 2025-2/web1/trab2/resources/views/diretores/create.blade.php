...
@section('content')
<div class="container">
  <h1>Adicionar Diretor</h1>
  <form action="{{ route('diretores.store') }}" method="POST">
    @csrf

    <div class="mb-3">
      <label for="nome" class="form-label">Nome do Diretor*</label>
      <input type="text" name="nome" id="nome" class="form-control" value="{{ old('nome') }}" required maxlength="50">
    </div>

    <div class="mb-3">
      <label for="nota_media" class="form-label">Nota Média</label>
      <input type="number" name="nota_media" id="nota_media" class="form-control" value="{{ old('nota_media') }}" min="0" max="10" step="0.1" placeholder="0.0 a 10.0">
      <div class="form-text">Nota de 0 a 10</div>
    </div>

    <div class="mb-3">
      <label for="idade" class="form-label">Idade</label>
      <input type="number" name="idade" id="idade" class="form-control" value="{{ old('idade') }}" min="1" max="150">
    </div>

    <div class="mb-3">
      <label for="premios" class="form-label">Número de Prêmios</label>
      <input type="number" name="premios" id="premios" class="form-control" value="{{ old('premios') }}" min="0">
    </div>
