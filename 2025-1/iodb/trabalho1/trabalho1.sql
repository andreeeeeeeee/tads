...
INSERT INTO conta (usuario_id, nome_usuario)
VALUES (1, '@andre_bs_'),
  (1, '@andrefutebois');
...
INSERT INTO publicacao (texto, arquivo_principal)
VALUES ('foto na agua', 'foto_na_agua.jpeg'),
  (
    'foto de paisagem',
    'foto_de_paisagem.png'
  ),
  (
    'dump trimestral',
    'dump_trimestral.gif'
...
INSERT INTO conta_publicacao (conta_id, publicacao_id)
VALUES (1, 1),
  (2, 2),
  (2, 3);
...
INSERT INTO comentario (publicacao_id, conta_id)
VALUES (1, 1),
  (1, 2),
  (3, 2),
  (3, 1);
...
INSERT INTO arquivo (publicacao_id, arquivo)
VALUES (1, 'foto_na_agua_2.jpeg'),
...
FROM publicacao p
  ...
  JOIN usuario u ON cnt.usuario_id = u.id
  JOIN comentario cmnt ON p.id = cmnt.publicacao_id
WHERE cmnt.publicacao_id IS NOT NULL
ORDER BY p.id,
  cmnt.data_hora;
...
WHERE cmnt.publicacao_id IS NULL
ORDER BY p.id;
...
WHERE arq.publicacao_id IS NULL
ORDER BY p.id;
...
SELECT cnt.id,
  COUNT(cmnt.id) AS quantidade_comentarios
FROM conta cnt
  JOIN comentario cmnt ON cnt.id = cmnt.conta_id
GROUP BY cnt.id
ORDER BY quantidade_comentarios DESC
LIMIT 1;
...
FROM usuario u
  JOIN conta cnt ON u.id = cnt.usuario_id
WHERE cnt.data_hora_criacao = (
    SELECT MAX(data_hora_criacao)
    FROM conta
  )
ORDER BY u.nome,
  cnt.nome_usuario;
...
GROUP BY u.nome
HAVING COUNT(cnt.id) = (
    SELECT MAX(quantidade_contas)
    FROM (
        SELECT COUNT(cnt.id) AS quantidade_contas
        FROM usuario u
          JOIN conta cnt ON u.id = cnt.usuario_id
        GROUP BY u.nome
      ) AS subquery
...
SELECT cmnt.id,
  cmnt.data_hora
FROM comentario cmnt
WHERE cmnt.data_hora >= NOW() - INTERVAL '7 days'
ORDER BY cmnt.data_hora DESC;
...
SELECT COUNT(cmnt.id) AS quantidade_comentarios
FROM comentario cmnt
WHERE cmnt.data_hora BETWEEN '2023-01-01' AND '2023-12-31'
ORDER BY cmnt.data_hora DESC;
...
GROUP BY p.id,
  p.texto
HAVING COUNT(arq.id) > 1
ORDER BY quantidade_arquivos DESC;
...
GROUP BY p.id,
  p.texto
HAVING LENGTH(p.texto) = (
    SELECT MAX(LENGTH(texto))
    FROM publicacao
  )
ORDER BY quantidade_caracteres DESC;
...
FROM usuario u
  ...
  JOIN conta_publicacao cp ON cnt.id = cp.conta_id
  JOIN publicacao p ON cp.publicacao_id = p.id
WHERE p.data_hora BETWEEN '2023-01-01' AND '2023-12-31'
GROUP BY u.nome
